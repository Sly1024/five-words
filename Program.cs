public class FiveWords {
    private static List<List<string>> alternatives = new();
    private static List<int> bitmasks = new();
    ...
    public static void Main() {
        ...
        pairs = GetWordPairs(bitmasks);

        const int chunksize = 50;
        var tasks = new List<Task>();

        for (int i = 0; i < bitmasks.Count - 4; i += chunksize) {
            var endIdx = Math.Min(i + chunksize, bitmasks.Count - 4);
            tasks.Add(Task.Run(TestWords(i, endIdx)));
        }

        Console.WriteLine("Chunks: " + tasks.Count);
    ...
    private static Action TestWords(int startIdx, int endIdx) {
        // this function can run entirely in parallel, no need to lock, because it only reads
        // the "global" state (bitmasks, pairs) and writes only to local variables
        return () => {
            ...
            for (int word1 = startIdx; word1 < endIdx; word1++) {
                solution[0] = word1;
    ...
    private static void ListSolutions(int[] solution) {
        // need to lock here, otherwise the console writes from different threads would get interleaved
        lock (alternatives) {
